...
if caseType ~=32
    ...
    for IES_no = 1 : 3
        ...
        xlabel(sprintf('EH%d', IES_no))
        ylabel('Load(MW)')
        if IES_no == 1
            H1 = legend('base electric load','total electric load','base thermal load','total thermal load','Location','northoutside','Orientation','horizontal');
            set(H1,'Box','off');
        end
        % ylabel('load / kW')
        % xlabel('time / h')
        ...
        hold on
        stairs(t, EH_solarP / 1000, 'Color', 'k', 'LineStyle', '-', 'LineWidth', w);
        stairs(t, EH_windP / 1000, 'Color', 'k', 'LineStyle', '--', 'LineWidth', w);
        xlim([0, 24*period]);
        xticks(0 : (24 * period /4) : 24 * period);
        xticklabels({'0:00','6:00','12:00','18:00','24:00'});
        xlabel(sprintf('EH%d', IES_no))
    ...
    le = legend([H1,H2(1),H2(2),H3(1)],'transformer power','local price','utility price','power limits'); set(le,'Box','off');
    ...
    for IES_no = st : en
        ...
        if IES_no == st
            le = legend([H1(1), H1(2), H1(3), H1(4), H2, H4(1), H4(2)], 'transformer','CHP','renewable energies','EES','SOC of EES','base electric load','total electric load','Location','northoutside','Orientation','horizontal');
            set(le, 'Box', 'off');
        end
    ...
    for IES_no = st : en
        ...
        if IES_no == st
            le = legend([H1(1),H1(2),H1(3),H2,H4(1),H4(2)], 'CHP','GF','TES','SOC of TES','base thermal load','total thermal load','Location','northoutside','Orientation','horizontal');
            set(le,'Box','off');
        end
...
%     set(H3(3),'Color',indianred,'LineWidth',1.5);
    ...
    le = legend([H1(1),H2(1),H3(1),H4(1)], 'EES+TES', 'TES','EES','no storages','Location','northoutside','Orientation','horizontal');
%     le = legend([H1(1),H2(1),H3(1),H4(1),H1(2),H1(3)], 'IES1', 'IES1 w/o ESS','IES1 w/o ThSS','IES1 w/o SS','IES2', 'IES3','Location','northoutside','Orientation','horizontal');
    set(le,'Box','off');
